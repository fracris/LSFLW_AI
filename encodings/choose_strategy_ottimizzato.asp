...
% ===== FATTI BASE E IDENTIFICAZIONE SISTEMI (OTTIMIZZATI) =====
% Cache dei sistemi per evitare ricalcoli ripetuti
...
% Connessioni ottimizzate - calcola una sola volta
undirected_connected(M, S) :- connected(M, S).
undirected_connected(M, S) :- connected(S, M).

% ===== ANALISI TATTICHE OTTIMIZZATE =====
% Cache dei sistemi di confine
...
% Cache sistemi ad alta produzione
high_production_system(S) :- production(S, P), P > 2.

% ===== CONTEGGI OTTIMIZZATI (CALCOLA UNA SOLA VOLTA) =====
...
% Riduci complessità delle condizioni storiche
...
% ===== CONDIZIONI STRATEGICHE SEMPLIFICATE =====
...
% Ottimizzazione: calcola enemy_near una sola volta
enemy_near(P) :- border_to_enemy(_, P).

% Semplifica direct_attack_conditions
direct_attack_conditions(P) :- enemy_near(P).

% Ottimizza cooperative_target - evita count costosi
cooperative_target(Enemy_S, P) :-
    enemy_system(Enemy_S, P),
    border_to_enemy(S1, P),
    border_to_enemy(S2, P),
    S1 != S2,
    connected(S1, Enemy_S),
    connected(S2, Enemy_S).

cooperative_attack_conditions(P) :- cooperative_target(_, P).

% ===== APPLICABILITÀ STRATEGIE (SEMPLIFICATE) =====
...
% ===== SELEZIONE STRATEGIE OTTIMIZZATA =====
...
% ===== AZIONI STRATEGICHE OTTIMIZZATE =====
target_neutral(From, To) :-
    ...
    ships(From, Ships),
    Ships > 20.  % Soglia fissa invece di > 0
...
% ATTACCO DIRETTO OTTIMIZZATO
% Cache del controllo flotte volanti
has_incoming_fleet(To, From) :- fleet(_, _, _, To, From, _).
...
direct_attack(From, To) :-
    ...
    FromShips > ToShips + (20 * ToProd),
    not has_incoming_fleet(To, From).
...
direct_attack(From, To) :-
    ...
    ships(To, ToShips),
    production(To, ToProd),
    fleet(_, _, IncomingShips, To, From, _),
    FromShips > ToShips + (20 * ToProd) + IncomingShips.

% ===== CALCOLO NAVI OTTIMIZZATO =====
...
attack_ships(From, To, Ships) :-
    ...
    ships(To, ToShips),
    production(To, ToProd),
    Ships = ToShips + (20 * ToProd),
    Ships > 0,
    Ships < FromShips.

% ===== ATTACCO COORDINATO SEMPLIFICATO =====
% Identifica il primo bersaglio valido invece di cercare il migliore
first_cooperative_target(To) :-
    chosen_strategy(cooperative_attack),
    enemy_system(To, P),
    cooperative_target(To, P).

% Semplifica la logica di attaccanti coordinati
simple_coordinated_attacker(From, To) :-
    chosen_strategy(cooperative_attack),
    first_cooperative_target(To),
    my_system(From),
    undirected_connected(From, To),
...
% ===== VINCOLI ESSENZIALI =====
:- send_attack_fleet(F1, T, _), send_cooperative_fleet(F2, T, _).
:- send_cooperative_fleet(F1, T1, _), send_cooperative_fleet(F2, T2, _), T1 != T2.
